...
if __name__ == '__main__':
    ...
    for i, line in enumerate(lines):
        ...
        for neighbor in neighbors:
            ns_flag = 0
            for sub in S:
                if (sub + ' ' in neighbor) or (sub + '\'' in neighbor) or (sub + ',' in neighbor) or (sub + '.' in neighbor):
                    neighbor_ppls = icl_lm_eval(model, tokenizer, icl_examples, [target_true, target_new],
                                            f'New Fact: {prompt} {target_new}\nPrompt: {neighbor}')
                    ns_flag = 1
                    break
            if ns_flag == 0:
